...
#define CONFIG_ESPNOW_CHANNEL 0 // keep as 0 to auto-detect in APSTA mode
...
int example_espnow_data_parse(uint8_t *data, uint16_t data_len, uint8_t *state, uint16_t *seq, uint32_t *magic) {
    auto *buf = (example_espnow_data_t *) data;
...
void example_espnow_data_prepare(example_espnow_send_param_t *send_param) {
    auto *buf = (example_espnow_data_t *) send_param->buffer;
...
static void example_espnow_task(void *pvParameter) {
    ...
    auto *send_param = (example_espnow_send_param_t *) pvParameter;
    if (esp_now_send(send_param->dest_mac, send_param->buffer, send_param->len) != ESP_OK) {
        ESP_LOGE(TAG, "Send error 1");
            ...
            case EXAMPLE_ESPNOW_RECV_CB: {
                ...
                if (ret == EXAMPLE_ESPNOW_DATA_BROADCAST) {
                    ...
                    if (!esp_now_is_peer_exist(recv_cb->mac_addr)) {
                        auto *peer = static_cast<esp_now_peer_info_t *>(malloc(
...
static esp_err_t example_espnow_init() {
    ...
    if (s_example_espnow_queue == nullptr) {
    ...
    auto *peer = static_cast<esp_now_peer_info_t *>(malloc(sizeof(esp_now_peer_info_t)));
    if (peer == nullptr) {
        ESP_LOGE(TAG, "Malloc peer information fail");
    ...
    send_param = static_cast<example_espnow_send_param_t *>(malloc(sizeof(example_espnow_send_param_t)));
    ...
    send_param->buffer = static_cast<uint8_t *>(malloc(CONFIG_ESPNOW_SEND_LEN));
    ...
    xTaskCreate(example_espnow_task, "example_espnow_task", 2048 + 4096, send_param, 4, NULL);
